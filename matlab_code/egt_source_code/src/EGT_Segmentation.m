...
%     -upper_hole_size_bound: black holes smaller than this in the segmented image will be filled in.
%     -manual_finetune: 
% 
% Outputs
%     -S: segmented image (logical)

function S = EGT_Segmentation(I, min_cell_size, upper_hole_size_bound, manual_finetune, erosion_val, dilation_val)
...
S = S > threshold;
S = imdilate(S,strel('disk',dilation_val));
S = fill_holes(S, upper_hole_size_bound);
S = imerode(S,strel('disk',erosion_val));
S = bwareaopen(S,min_cell_size,4); % removes small objects from image
end